...
stage_dir := $(project_dir)/stage-dir
...
$(build_dir)/spdlog.marker:
	...
	cmake -Wno-dev --trace-expand --trace-redirect=$(build_dir)/spdlog/cmake.log -DCMAKE_INSTALL_PREFIX=$(prefix_dir) -DCMAKE_BUILD_TYPE=Release -DCMAKE_PREFIX_PATH="$(prefix_dir);$$MINGW_PREFIX" -B $(build_dir)/spdlog -S $(download_dir)/spdlog
	cmake --build $(build_dir)/spdlog --config Release --target install
...
windows-mingw: folders
	eval "$$windows_mingw_makefile" > $(build_dir)/windows-mingw.Makefile
	$(MAKE) -f $(build_dir)/windows-mingw.Makefile battery version artifact
